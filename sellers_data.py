...
def sellers_data(prd_name: str, suppliers_qty: int) -> list:
    ...
    for _, prod_containers in zip(range(suppliers_qty), prod_containers):
        ...
        data_list.append(
            [
                company_name.text if company_name != None else "",
                main_prods.text if main_prods != None else "",
                company_name["href"] if company_name != None else "",
                dir_cont_link["href"] if dir_cont_link != None else "",
            ]
        )
    return data_list
